...
class AndroidVideoPlayer extends VideoPlayerPlatform {
  ...
  @override
  Future<void> setPictureInPictureActions(int playerId, List<PictureInPictureAction> actions) {
    final List<PipAction> pipActions = actions.map((PictureInPictureAction a) {
      return PipAction(type: _pipActionTypeFromPictureInPictureActionType(a.type), label: a.label);
    }).toList();
    return _api.setPictureInPictureActions(playerId, pipActions);
  }

  PipActionType _pipActionTypeFromPictureInPictureActionType(PictureInPictureActionType type) {
    return switch (type) {
