...
public final class VideoPlayerPlugin: NSObject, FlutterPlugin, AVFoundationVideoPlayerApi {
  ...
  func createTexturePlayer(options creationOptions: CreationOptions) throws -> TexturePlayerIds {
    ...
    let playerId = configurePlayer(
      player,
      extraDisposeHandler: { [weak self] in
  ...
  private func configurePlayer(
    ...
    if let backgroundPlayback = backgroundPlayback {
      player.configureBackgroundPlayback(toObjcBackgroundPlayback(backgroundPlayback))
    }
  ...
  private func toObjcBackgroundPlayback(
    _ message: BackgroundPlaybackMessage
  ) -> FVPBackgroundPlaybackMessage {
    var notificationMetadata: FVPNotificationMetadataMessage?
    if let metadata = message.notificationMetadata {
      notificationMetadata = FVPNotificationMetadataMessage.make(
        withId: metadata.id,
        title: metadata.title,
        album: metadata.album,
        artist: metadata.artist,
        durationMs: metadata.durationMs.map { NSNumber(value: $0) },
        artUri: metadata.artUri
      )
    }
    return FVPBackgroundPlaybackMessage.make(
      withEnableBackground: message.enableBackground,
